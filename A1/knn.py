...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
      ...
      for j in range(num_test):
        y_flat = x_test[j].flatten()
        dists[i][j] = torch.sum((x_flat-y_flat)*(x_flat-y_flat))
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
      ...
      diff = torch.sum((x_test_re - x_flat)**2,dim = 1)
      dists[i] = diff
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_train_dist = x_train.reshape(num_train,-1)
    x_test_dist = x_test.reshape(num_test,-1)
    x_train_dist = torch.sum(x_train_dist**2,dim =1,keepdim=True)
    
    x_test_dist  = torch.sum(x_test_dist**2,dim =1)
    # train = torch.sum(x_train_dist,dim =1)
    # test = torch.sum(x_test_dist,dim =1)
    # print(train.shape)
    # print(x_test_dist.t().shape)
    
    x_train_re = x_train.reshape(x_train.shape[0],-1)
    x_test_re = x_test.reshape(x_test.shape[0],-1).t()
    
    matrix       = 2* torch.matmul(x_train_re,x_test_re)
    
    
    dists = x_train_dist +x_test_dist - matrix
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    for j in range(num_test):
        ...
        # Assign the majority label to the j-th test sample
...
def knn_cross_validate(
    ...
    train = list(x_train.chunk(num_folds))
    y_test = list(y_train.chunk(num_folds))
    ...
    for i in range(len(k_choices)):
      for j in range(num_folds-1):
        classifier = KnnClassifier(train[j],train[num_folds-1])
        # pred = classifier.predict(train[num_folds-1])
        # print(y_test[j].shape)
        # print(train[j].shape)
        accuracy = classifier.check_accuracy(train[j],y_test[j],k_choices[i])
        k_to_accuracies[f'{j}_{k_choices[i]}'] = accuracy
    ...
    #     classifier = KnnClassifier(train[j],y_test[j])
    #     pred = classifier.predict(y_test[j])
